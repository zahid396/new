...
class CheckoutController extends Controller
{
    public function show(int $productId)
    {
        ...
        $paymentMethods = PaymentSetting::where('is_active', true)->get()
            ->groupBy('method')
            ->mapWithKeys(fn ($group, $method) => [$method => $group->first()]);

        return view('checkout.show', compact('product', 'session', 'paymentMethods'));
    }
    ...
    public function submit(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($validated, $product, $amount, $orderId, $session) {
                Order::create([
                    'order_id' => $orderId,
                    'product_id' => $product->id,
