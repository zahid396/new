...
/* Search form */
.track-search { max-width: 480px; margin: 0 auto 3rem; background: var(--white); border-radius: var(--radius-lg); padding: 2rem; box-shadow: var(--shadow); border: 1px solid var(--gray-100); }
.track-search-form { display: flex; gap: .75rem; }
.track-search-form input { flex: 1; padding: .85rem 1rem; border: 1px solid var(--gray-200); border-radius: var(--radius); font-size: 1rem; color: var(--gray-800); transition: var(--transition); }
.track-search-form input:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(99,102,241,.12); }
.track-search-form input::placeholder { color: var(--gray-400); }
.track-search-form button { padding: .85rem 1.75rem; background: linear-gradient(135deg, var(--primary), #8b5cf6); color: var(--white); font-weight: 700; font-size: .95rem; border: none; border-radius: var(--radius); cursor: pointer; transition: var(--transition); white-space: nowrap; }
.track-search-form button:hover { opacity: .92; }
.track-search .error-msg { color: #ef4444; font-size: .88rem; margin-top: .75rem; text-align: center; }
...
.timeline-step .step-dot.active { background: #fef3c7; color: #d97706; box-shadow: 0 0 0 4px rgba(217,119,6,.15); }
.timeline-step .step-dot.done { background: var(--accent); color: var(--white); }
.timeline-step .step-dot.rejected { background: #fecaca; color: #dc2626; }
...
.timeline-step.rejected-step .step-label { color: #dc2626; }
.timeline-step.rejected-step .step-desc { color: #dc2626; font-size: .82rem; margin-top: .25rem; }

/* Status-specific summary */
.status-summary { text-align: center; padding: 1.5rem; border-radius: var(--radius); margin-bottom: 1.5rem; font-weight: 700; font-size: 1rem; }
.status-summary.verified { background: #ecfdf5; color: #059669; }
.status-summary.pending { background: #fef3c7; color: #d97706; }
.status-summary.rejected { background: #fef2f2; color: #dc2626; }
        ...
        <div class="track-result">
            ...
            <div class="order-info-card">
                ...
                <div class="info-rows">
                    ...
                    <div class="info-row"><span class="label">Status</span><span class="value"><span class="status-badge" style="background:{{ in_array($statusLower,['delivered','verified','completed'])?'#ecfdf5':($statusLower==='rejected'?'#fef2f2':'#fef3c7') }};color:{{ in_array($statusLower,['delivered','verified','completed'])?'#059669':($statusLower==='rejected'?'#dc2626':'#d97706') }};">{{ strtoupper($status) }}</span></span></div>
                    @if($statusLower === 'rejected' && !empty($order->rejection_reason))
                        <div class="info-row"><span class="label">Rejection Reason</span><span class="value" style="color:#dc2626;">{{ $order->rejection_reason }}</span></div>
                    @endif
                </div>
            </div>
