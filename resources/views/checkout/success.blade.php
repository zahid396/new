...
.success-card { max-width: 560px; margin: 0 auto; background: var(--white); border-radius: var(--radius-xl); padding: 3rem 2.5rem; text-align: center; box-shadow: var(--shadow-md); border: 1px solid var(--gray-100); }
.success-icon { width: 72px; height: 72px; border-radius: 50%; background: #ecfdf5; color: #10b981; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.25rem; font-size: 2rem; animation: popIn .4s cubic-bezier(.175,.885,.32,1.275); }
...
.order-id-box { display: inline-block; background: var(--primary-50); border: 2px dashed var(--primary-light); border-radius: var(--radius); padding: .75rem 2rem; margin-bottom: 2rem; }
...
.status-badge { display: inline-block; padding: .25rem .75rem; border-radius: 50px; font-size: .78rem; font-weight: 700; background: #fef3c7; color: #d97706; }
.btn-track-order { display: inline-flex; align-items: center; gap: .5rem; padding: .85rem 2rem; background: linear-gradient(135deg, var(--primary), #8b5cf6); color: var(--white); font-weight: 700; font-size: 1rem; border: none; border-radius: var(--radius); cursor: pointer; transition: var(--transition); text-decoration: none; }
.btn-track-order:hover { opacity: .92; transform: translateY(-2px); color: var(--white); box-shadow: var(--shadow-lg); }
